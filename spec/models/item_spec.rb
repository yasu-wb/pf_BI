...
RSpec.describe Item, type: :model do
  ...
  context '製品登録ができない時' do
    ...
    it 'INコードが6桁未満では登録できない' do
      @item.incode = 12345
      @item.valid?
      expect(@item.errors.full_messages).to include('INコードは6桁で入力してください')
    end

    it 'INコードが7桁以上では登録できない' do
      @item.incode = 1234567
      @item.valid?
      expect(@item.errors.full_messages).to include('INコードは6桁で入力してください')
    end
